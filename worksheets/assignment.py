...
N = 20
P = 200
MUTATION_RATE = 0.05  # 0.04  # 0.05
MUTATION_STEP = 1 # 0.85  # 0.80
MIN = -100.0  # 0.0
MAX = 100.0   # 1.0

"""
def move_points(number, shift, base=10):
...
def generate_genes():
    ...
    for i in range(0, P):
        ...
        new_ind.fitness = minimisation_func1(new_ind)
...
def generate_fitness(population):
    for ind in population:
        ...
        ind.fitness = minimisation_func1(ind)
    return population
...
def crossover_test(offspring):
    for i in range(0, P, 2):
        ...
        off1.fitness = minimisation_func1(off1)
        off2.fitness = minimisation_func1(off2)
...
def mutation(offspring):
    for i in range(0, P):
        ...
        new_individual.fitness = minimisation_func1(new_individual)  # new
...
def minimisation_func1(individual) -> float:
    fitness = 0
...
def test(population, offspring):
    ...
    new_population[-1] = bestIndividual  # Changed to 0 (best) from -1 (worst)
    offspring.clear()
...
def iterate(population):
    ...
    for i in range(generations):
        ...
        minORmax_fitness = min(fitness)  # min to max
        #print(move_points(minORmax_fitness, -10))
        # best_fitness.append(best_population_fitness(population))
        mean_fitness = (sum(fitness) / P)

        best_fitness.append(minORmax_fitness)
